...
public final class Thermometer extends View {
	...
	private Bitmap faceTextureRed;
	private Bitmap faceTextureYellow;
	private Paint facePaint;
	private Paint facePaintRed;
	private Paint facePaintYellow;
	...
	private String scaleUpperTitle = "Nuked";
	//private String scaleLowerTitle = "Boom";
	...
	private void initDrawingTools() {
		...
		BitmapShader paperShader = new BitmapShader(faceTexture, 
												    ...
												    Shader.TileMode.MIRROR);
		faceTextureRed = BitmapFactory.decodeResource(getContext().getResources(), 
				   R.drawable.plasticred);
		BitmapShader paperShaderRed = new BitmapShader(faceTextureRed, 
												    Shader.TileMode.MIRROR, 
												    Shader.TileMode.MIRROR);
		faceTextureYellow = BitmapFactory.decodeResource(getContext().getResources(), 
				   R.drawable.plasticyellow);
		BitmapShader paperShaderYellow = new BitmapShader(faceTextureYellow, 
												    Shader.TileMode.MIRROR, 
												    Shader.TileMode.MIRROR);
		...
		paperMatrix = new Matrix();
		facePaintRed = new Paint();
		facePaintRed.setFilterBitmap(true);
		paperMatrix.setScale(1.0f / faceTextureRed.getWidth(), 
							 1.0f / faceTextureRed.getHeight());
		paperShaderRed.setLocalMatrix(paperMatrix);
		facePaintRed.setStyle(Paint.Style.FILL);
		facePaintRed.setShader(paperShaderRed);
		
		paperMatrix = new Matrix();
		facePaintYellow = new Paint();
		facePaintYellow.setFilterBitmap(true);
		paperMatrix.setScale(1.0f / faceTextureYellow.getWidth(), 
							 1.0f / faceTextureYellow.getHeight());
		paperShaderYellow.setLocalMatrix(paperMatrix);
		facePaintYellow.setStyle(Paint.Style.FILL);
		facePaintYellow.setShader(paperShaderYellow);
		...
		logo = BitmapFactory.decodeResource(getContext().getResources(), R.drawable.nuclear);
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		setMeasuredDimension(chosenDimension, chosenDimension / 2);
	}
	...
	private void drawRim(Canvas canvas) {
		...
		canvas.drawArc(rimRect, 0, -180, false, rimPaint);
		// now the outer rim circle
		//canvas.drawOval(rimRect, rimCirclePaint);
		canvas.drawArc(rimRect, 0, -180, false, rimCirclePaint);
	}
	
	private void drawFace(Canvas canvas) {		
		//canvas.drawOval(faceRect, facePaint);
		canvas.drawArc(faceRect, 0, -180, false, facePaint);
		
		//Custom color fields
		canvas.drawArc(scaleRect, -65, 180, false, facePaintYellow);
		canvas.drawArc(scaleRect, -20, 180, false, facePaintRed);
		// draw the inner rim circle
		//canvas.drawOval(faceRect, rimCirclePaint);
		canvas.drawArc(faceRect, 0, -180, false, rimCirclePaint);
		// draw the rim shadow inside the face
		//canvas.drawOval(faceRect, rimShadowPaint);
		canvas.drawArc(faceRect, 0, -180, false, rimShadowPaint);
	}
